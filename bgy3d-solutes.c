...
/* Read density  file generated by  GPAW calculation need  ANGSTROM to
 * scale the length values back */
#define ANGSTROM 0.52917725750691647
static void read_charge_density (DA da, const ProblemData *PD,
                                 const char *filename, real fact, Vec v)
{
  PetscScalar ***vec;
  ...
  /* electron density scaled by ANGSTROM^3 in python script */
  real invB3 = 1. / ANGSTROM / ANGSTROM / ANGSTROM;
  for (int i = i0; i < i0 + ni; i++)
    for (int j = j0; j < j0 + nj; j++)
      for (int k = k0; k < k0 + nk; k++)
        {
