...
\documentclass[english,twocolumn,showkeys,superscriptaddress]{revtex4}
...
\affiliation{Department Chemie and Catalysis Research Center, Technische Universität
...
Matveev, Krüger, Rösch, \emph{Comp. Theor. Chem.} \textbf{1051}, p.~151,
...
and the best theoretical estimates. 
\end{abstract}

\keywords{Reference interaction site model, aqueous solution, uranyl, solvation}
...
of used nuclear fuel \cite{book:Morss06,book:Grenthe92,Silva95}.
...
using empiric parameters \cite{Tomasi05,Fuchs02}. The state of the
art PCM is owing its success in part to the conductor-like screening
model (COSMO) of electrostatic interactions \cite{Klamt95} and has
successfully been applied in various studies of uranyl complexes \cite{Fuchs02,Moskaleva04,Cao05,Shamov05,Gutowski06}.
...
solvent description \cite{Guilbaud93,Guilbaud96,Rai12,Pomogaev13,Kerisit13,Tiwari14}.
Though several studies \cite{Gordon00,hagberg05,Frick09,Tirler13}
...
demanding QM dynamic simulations \cite{Buehl05,Buehl06,Buehl11,Nichols08}
are restricted to comparably small numbers of solvent molecules. 

Meanwhile, the integral equation theories of molecular liquids \cite{book:Hirata03,book:Hansen06},
...
interaction site model (RISM) \cite{Chandler72,Hirata81,Hirata82,Kinoshita97}
and its three-dimensional generalization (3D RISM) \cite{Beglov95,Kovalenko98}
...
\cite{Singer85,Kast08} and, hence, intuitive expressions for the
first derivatives. The dielectrically consistent RISM (DRISM) \cite{Perkyns92}
was designed to compensate the error of the gas-like dielectric response
of RISM for electrolytes and ionic solutes \cite{Perkyns94,Chiodo07,Chuev08,Joung13}.
...
to simulate finite systems \cite{Makov95}. It was understood early
...
as well \cite{Ng74}. For 3D RISM the few practical approaches include
the background charge correction technique \cite{Kovalenko00b}, monopole
correction \cite{Kloss08b}, and an exact re-summation for a superposition
of gaussian charges \cite{Perkyns10}. In this work we present a generalization
of the exact re-summation technique for arbitrary solute charge densities.
...
\cite{Tenno93,Tenno94}. An extension to 3D RISM SCF was developed
for density functional theory (DFT) \cite{Kovalenko99} and \emph{ab
initio} calculations \cite{Kloss08}. The analytical gradients for
the self-consistent combination of 3D RISM and DFT \cite{Gusarov06}
...
and quantum chemistry are discussed in the recent review by Sato \cite{Sato13}.
...
be written in the following form \cite{book:Hirata03}:
...
number density $\rho$. \cite{book:Hirata03}. The star denotes the
...
\cite{Chandler72,book:Hirata03}. The solvent susceptibility as a
...
to as a renormalized indirect correlation \cite{Kast08}:
\begin{equation}
f(x)=\begin{cases}
\mathrm{e}^{x}-1 & x\le0\\
...
of the equations \cite{Kovalenko99}. Higher order closures with $n>1$
are referred to as partial series expansion closures of order $n$
(PSE$n$). For consistency, we will also refer to the KH closure as
PSE1 in this work.
...
Lorentz--Berthelot rules \cite{book:Hirata03}. The Ewald range parameter
$a=1.2$~Å$^{-1}$ is common for all site pairs. Further, to circumvent
...
Singer and Chandler (here in temperature units) \cite{Singer85}:
\begin{equation}
\mu_{\text{HNC}}=\rho\int d^{3}r\sum_{\alpha}\left[h_{\alpha}^{2}/2-c_{\alpha}-c_{\alpha}h_{\alpha}/2\right].\label{eq:chem-pot-hnc}
\end{equation}
A modification for the KH closure \cite{Kovalenko99} and a generalization
...
$x>0$ and zero otherwise \cite{Kast08}. In the case of the KH (PSE1)
...
field $v$ \cite{Singer85,Kast08}. This property leads to straightforward
...
not on the solvent site distribution functions $h$ \cite{Bo15}. 
...
molecule which is about $-$9.9~kcal/mol \cite{Berendsen87,Mahoney00}
to be compared to the solvation energy of $-$6.3~kcal/mol derived
from the ratio of the equilibrium vapor/liquid concentrations \cite{Bridgeman64}.
...
\cite{petsc-user-ref} with the analytic matrix-free Jacobian, $J[t]=\delta F/\delta t$,
...
scheme \cite{Ng74}, separate the smooth asymptote $v_{\text{L}}$
...
$\kappa$ (here in temperature units) \cite{book:Hansen06}. More
...
dielectric correction of the DRISM approach \cite{Perkyns92,Cummings81,Kovalenko00b}.
...
these points by construction \cite{FFTW05}. The solute part is the
...
\cite{Kloss08b} and to a superposition of gaussian charge distributions
\cite{Perkyns10}.  The argument line used above assumes the net
...
discussion of the background charge correction in Ref.~\cite{Kovalenko00b}.
...
To model uranyl solvation we employ the SPC/E water model \cite{Berendsen87},
modified as suggested by Pettitt and Rossky \cite{Pettitt82} by addition
of a Lennard--Jones (LJ) term for hydrogen atoms with $\sigma_{\text{H}}=0.4$~Å
and $\epsilon_{\text{H}}=0.046$~kcal/mol (PR-SPC/E). For atomic
ions we adopted the coincident SPC/E water model (cSPC/E) with $\sigma_{\text{H}}=1.1658$~Å
and $\epsilon_{\text{H}}=0.01553$~kcal/mol as proposed by Luchko
and Kovalenko \cite{Luchko10} in order to compare with published
...
interaction with the uranium charge of 3.5~\emph{e} (KL2) \cite{Kerisit13}.
For convenience the reader may refer to our previous work \cite{Bo15}
...
particle density $\rho=0.0333295$~Å$^{-3}$. These parameters correspond
to a pressure of 1~atm. The 3D domain was represented by a cubic
cell of $20^{3}$~Å$^{3}$ and a Cartesian grid with 96$^{3}$ points
and a resolution of about 0.208~Å. The radial grid used to precompute
...
radial range of 40~Å and 1536 points corresponding to a resolution
of about 0.026~Å. 
...
\cite{Dunlap90} as implemented in the parallel code ParaGauss \cite{ParaGaussV3.1}.
The atomic approximation \cite{Matveev08} to the second-order Douglas\textendash{}Kroll\textendash{}Hess
scalar-relativistic approach \cite{rev:roesch96} was used to account
...
GGA) \cite{Goerling99,kochDFT}, as suggested by Becke and Perdew
(BP) \cite{Becke88,Perdew86c}. The Kohn\textendash{}Sham orbitals
...
the same as in earlier studies on uranyl species \cite{Moskaleva04,Moskaleva06b,Moskaleva06a,Kremleva12b,Bo15}. 

We used a COSMO PCM solvation model for comparison \cite{Fuchs02},
with the cavity defined via an effective solvent radius of 1.4~Å
and van der Waals radii of solute atoms as tabulated by Bondi \cite{Bondi64}
and scaled by 1.125 (except for hydrogen). We carried out the cavity
tessellation with the FIXPVA approach \cite{Su09,Kremleva12b} (fixed
points with variable areas) to obtain numerically stable energies
and forces \cite{Kremleva12b}. 
...
the utility package ParaTools \cite{Chaffey-Millar11,Nikodem13}.
...
\cite{Moskaleva04,Cao05,Buehl05}. However, for a study of the water
exchange with MM+RISM we adopted a flexible uranyl model without symmetry
constraints and compared two sets of intra-molecular parameters \cite{Kerisit13,Tiwari14}
...
ions \cite{Joung13,Bo15} applying the cSPC/E water model \cite{Berendsen87,Luchko10}.
With moderate settings  for the grid resolution of $64^{3}$ points
in total and a cubic box volume of $20^{3}$~Å$^{3}$ we are able
to match the 1D results up to 0.3~kcal/mol or better. The accuracy
is slightly better for small cations than for the larger anions. The
values are comparable to MD with SPC/E water and experimental results.
The agreement may be slightly improved by a correction proportional
to the molar volume of the solute \cite{Sergiievskyi14} as was demonstrated
in our previous work \cite{Bo15}. 
...
3D HNC & $-$111.8 & $-$86.3 & $-$69.3 & $-$79.8 & $-$74.1 & $-$64.7\tabularnewline
...
\footnotetext{$^{a}$~Energies in kcal/mol. KH and HNC closure relations
have been used. 3D domain is $20^{3}$ Å$^{3}$ / $64^{3}$ pts. $^{b}$~Ref.~\cite{Bo15}.
$^{c}$~MD with SPC/E water from Ref.~\cite{Joung08}. $^{d}$~Ref.~\cite{Schmid00}.} 
\end{table}
...
\cite{Bo15}, the MM+RISM combination features a vacancy in the equatorial
...
\footnotetext{ $^{a}$~Bond lengths in pm, energies in kcal/mol.
$^{b}$~GP structure is $D_{4\text{h}}$ but the RISM structures
feature a vacancy in the equatorial plane corresponding to the missing
ligand, see Figure~\ref{fig:spatial-distribution-4w}. $^{c}$~Five-coordinated
structure with the sixth water in the second shell. $^{d}$~Value
not available due to divergence. $^{e}$~The GP row shows binding
energies of $n$ water molecules to a rigid uranyl with $d$(U--O)
of 179~pm. }
\end{table}
\begin{figure}
...
\includegraphics[width=1\linewidth]{uo2-II\lyxdot d/4w}
\end{figure}
...
In our previous work exploring these models with 1D RISM \cite{Bo15}
...
approach did not show this effect convincingly \cite{Bo15}. Again,
...
from our previous work \cite{Bo15}. However, the relative order of
the estimates based on the four- and six-coordinated complexes is
not reproduced. The estimate of $\Delta G_{\text{sol}}$ based on
the four-coordinated complex is by $\sim$10~kcal/mol less negative
...
1D RISM \cite{Bo15}, which is remarkably close to the free energy
...
x-ray scattering (HEXS) experiment \cite{Soderholm05}. Given the
...
five-coordinated uranyl model appears to be too low \cite{Moskaleva04}
in the view of most recent experimental estimate of $-369\pm15$~kcal/mol
\cite{Gibson05}. The corresponding QM+RISM solvation energy of $-389$~kcal/mol
...
\footnotetext{ $^{a}$~Bond lengths in pm, energies in kcal/mol.
Symmetric uranyl aqua complexes {[}UO$_{2}$(H$_{2}$O)$_{n}${]}$^{2+}$,
$4\leq n\leq6$ and a bare uranyl ($n=0$) are treated by QM while
interactions with the PR-SPC/E water solvent are treated by KL2 force
field. $^{b}$~$D_{4\text{h}}$ structure. $^{c}$~$D_{5\text{h}}$
structure. $^{d}$~$D_{3\text{d}}$ structure. $^{e}$~The row shows
binding energies of $n$ water ligands. }
\end{table}
...
\includegraphics[width=1\linewidth]{uo2-II\lyxdot d/fig2}
\end{figure}
...
\includegraphics[width=1\linewidth]{uo2-II\lyxdot d/fig3}
\end{figure}
...
is about 9~kcal/mol \cite{Farkas00}. Many theoretical studies predict
much faster water exchange than the available experimental data suggests
\cite{Vallet01,hagberg05,Frick09,Nichols08,Kerisit10,Tiwari14}. A
simple way to obtain an estimate of the activation energy for the
...
5~kcal/mol derived from the residence time by Tiwari et al.~\cite{Tiwari14},
...
model \cite{Vallet01}. A hypothetical water exchange in the equatorial
...
\includegraphics[width=1\linewidth]{uo2-II\lyxdot d/pmf}
\end{figure}
...
\footnotetext{$^{a}$~Distances in pm, energies in kcal/mol. Locations
$r_{\text{a}}$ and $r_{\text{c}}$ correspond to the minima of first-
and second solvation shells, respectively; $r_{\text{b}}$ is the
highest point of the barrier connecting the two minima. } 
\end{table}
...
\includegraphics[width=1\linewidth]{uo2-II\lyxdot d/pmf-2d}
\end{figure}
...
\includegraphics[width=1\linewidth]{uo2-II\lyxdot d/profiles}
\end{figure}
...
rate \cite{Tiwari14}. The two intra-molecular force fields for a
...
and $d_{0}$(U--O) of 176~pm \cite{Tiwari14}. The alternative ``hard''
...
bending mode of 393~cm$^{-1}$ and a somewhat too long a uranyl bond
 $d_{0}$(U--O) of 180~pm \cite{Kerisit13}. The bending vibration
in crystals and solution has been experimentally characterized by
frequencies in the range of 180--270~cm$^{-1}$ \cite{Denning76,Denning79,Toth81,Gal92}.
...
range \cite{Matveev08}. For the gas phase uranyl the standard DFT
predicts too soft a bending mode likely because of the self-interaction
error for $f$-electrons \cite{Ramakrishnan11} while hybrid DFT and
CCSD(T) estimate the bending frequency around 160--180~cm$^{-1}$
\cite{Jackson08}. In this light the force field with the ``soft''
...
energy is characterized by $q>0.6$~Å for both the QM and the ``soft''
...
here and in other works \cite{Tiwari14,Kerisit13} and should be considered
a deficiency of the parametrization.  In turn, the ``soft'' model
does not convincingly show the preference for five-coordinated uranyl
with the sixth water in the second solvation shell over the six-coordinated
broken symmetry metastable structure. The QM and ``hard'' force
field models predict the six-coordinated $D_{3\text{d}}$-like state
by 5.6~kcal/mol and 3.6~kcal/mol less stable than the five-coordinated
...
is consistent with the findings of the other theoretical studies \cite{Tsushima07,Vallet01,Buehl06b,Rotzinger07,Tiwari14}
...
\cite{Farkas00}.  A comparison of locations and orientations of
...
of theory, and solvation model \cite{Tiwari14}. Tsushima \cite{Tsushima07}
...
4.5~kcal/mol in an earlier study \cite{Vallet01}, yet both are lower
than the experimental result. The comparison of PCM and a spherical
cavity self-consistent reaction field (SCRF) solvation models is reported
by Rotzinger \cite{Rotzinger07} using the complete active space self-consistent
field (CAS SCF) technique. The corresponding energy barriers of the
associative pathway are equal to 9.4~kcal/mol and 6.5~kcal/mol,
respectively. Water exchange energy barrier based on the thermodynamic
sampling is also estimated by Bühl et.\ al \cite{Buehl06b} using
the \textit{ab initio} molecular dynamics (AIMD). Their result of
6.7~kcal/mol is however lower than the experiment and the QM+RISM
...
Universität München. This work was partially supported by Bundesministerium
für Bildung und Forschung (grant No.\ 01RC1106A).
