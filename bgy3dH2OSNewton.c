...
/*   PetscPrintf(PETSC_COMM_WORLD, " %e \t", norm); */
  ...
  Compute_H2O_interS(BHD,
		     BHD->fg2HO_fft, gO, BHD->rhos[1], dgH);
  Compute_H2O_interS(BHD,
		     BHD->fg2HH_fft, gH, BHD->rhos[0], help2);
...
/*   VecCopy(gH, dgH); */
  ...
  VecCopy(dgO, help);
  Compute_H2O_interS(BHD,
		     BHD->fg2OO_fft, gO, BHD->rhos[1], dgO);
  Compute_H2O_interS(BHD,
		     BHD->fg2HO_fft, gH, BHD->rhos[0], help2);
  ...
  while(fu>1.0e-6 && iter<300)
    {
      ...
      VecCopy(dgO, gradO);
      VecCopy(dgH, gradH);
/*       VecAXPY(gradO, 1.0, BHD->g_ini[1]); */
/*       VecAXPY(gradH, 1.0, BHD->g_ini[0]); */
...
/*       VecAXPY(BHD->g_ini[1], -0.1*dcO, BHD->g_ini[1]); */
/*       VecAXPY(BHD->g_ini[0], -0.1*dcH, BHD->g_ini[0]); */
